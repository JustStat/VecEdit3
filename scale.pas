...
type
  ...
  public
    ...
    procedure SetJPegZoom(ARect: TDoubleRect; bmpWidth, bmpHeght: Integer);
...
procedure TZoomer.SetJPegZoom(ARect: TDoubleRect; bmpWidth, bmpHeght: Integer);
begin
  with ARect do begin
    if (abs(R - L) = 0) or (abs(D - U) = 0) then exit;
     Zoom := Min(bmpWidth / abs(R - L), bmpHeght/ abs(D - U));
     Zoom := EnsureRange(Zoom, MinZoom, MaxZoom);
     Offset.X := Min(Round(ARect.L * Zoom), Round(ARect.R * Zoom));
     Offset.Y := Min(Round(ARect.U  * Zoom), Round(ARect.D * Zoom));
  end;
end;
...
begin
  with ARect do begin
    if (abs(R - L) = 0) or (abs(D - U) = 0) then exit;
     Zoom := Min(PBSize.X / abs(R - L), PBSize.Y / abs(D - U));
     Zoom := EnsureRange(Zoom, MinZoom, MaxZoom);
     Offset.X := Min(Round(ARect.L * Zoom), Round(ARect.R * Zoom));
     Offset.Y := Min(Round(ARect.U  * Zoom), Round(ARect.D * Zoom));
  end;
